...
# 이진수 특성 : n을 2로 계속 나누었을 때 결국 나머지가 0 이 되면 2^0 즉 이진수 가장 오른쪽 비트가 0이 된다.
# 문제에서 요구하는 비트가 다른 지점이 2개 이하인 가장 작은 수는 ex) 0111 -> 1011 , 10011 -> 10101
# 처음 등장하는 0이 등장하기 이전(오른쪽) 비트에 해당하는 수(2^(cnt-1))를 더하는 것과 같다. 
    answer = []
    for n in numbers :
        k = n
        cnt = 0
        while k % 2 == 1:   # 이진수에서 0이 최초 등장하는 자리를 알아낸다.
            cnt += 1        # 0의 위치
            k //= 2
